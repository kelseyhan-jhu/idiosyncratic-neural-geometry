...
def main():    
    args = argparse.Namespace(
        dir="cov_occipital_functional_perm",
        roi="occipital",
        metric="cov",
    ...
    ANALYSIS_DIR = Path("/data/chan21/idiosyncratic-neural-geometry/results") / args.dir 
    ...
    if not (ANALYSIS_DIR / "eigenspectra.h5").exists():
        aggregator = SpectraAggregator(input_path=ANALYSIS_DIR, 
    ...
    if config.perform_permutations:
        ...
        cache_perm = DataCache(
            cache_dir = "/data/chan21/idiosyncratic-neural-geometry/data/cache_perm",
            enabled=True,
            )
        ...
        for perm_idx in tqdm(range(config.n_permutations)):
            ...
            if args.perform_downsampling:
                file = f"{cache_perm.cache_dir}/bin_pairwise_{args.roi}_{args.metric}_{str(args.n_downsample)}_{str(perm_idx)}_{','.join(config.movies)}.pkl"
            else:
